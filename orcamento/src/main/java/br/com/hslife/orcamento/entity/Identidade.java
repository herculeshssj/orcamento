...
import br.com.hslife.orcamento.rest.json.IdentidadeJson;
...
public class Identidade extends EntityPersistence {
	...
	private void validTituloEleitor() {
		...
		if (!resultado) {
			...
			if (!resultado){
				throw new ValidationException("Todos os campos do título de eleitor são obrigatórios!");
			}
		}
	}

	private void validRg() {
	...
	private void validPassaporte() {
		...
		if (!resultado) {
			...
			if (!resultado) {
				throw new ValidationException("Todos os campos do passaporte são obrigatórios!");
			}
		}
	}

	private void validDocMilitar() {
		...
		if (!resultado) {
			...
			if (!resultado) {
				throw new ValidationException("Todos os campos do certificado de reservista são obrigatórios!");
			}
		}
	}

	private void validCpf() {
		...
		try {
			...
			throw new ValidationException(ise.getMessage());
		}
				
	}

	private void validCnh() {
		...
		if (!resultado) {
			...
			if (!resultado)
				throw new ValidationException("Todos os campos da carteira de motorista são obrigatórios!");
		}
	}

	private void validCertidaoNascimento() {
		...
		if (!resultado) {
			...
			if (!resultado) {
				throw new ValidationException("Todos os campos da certidão de nascimento são obrigatórios!");
			}
		}
	}

	private void validCarteiraTrabalho() {
	...
	@Override
	public IdentidadeJson toJson() {
		return new IdentidadeJson();
	}
