...
-- Após a execução deste script, siga as recomendações do arquivo README.md do projeto "orcamento-maven" para poder prosseguir com a criação da base
-- de dados.
...
-- senha: admin
insert into usuario(ativo,dataCriacao,login,nome,senha,tipoUsuario) values (true, '2012-01-01', 'admin', 'Administrador do sistema' , 'd033e22ae348aeb5660fc2140aec35850c4da997', 'ROLE_ADMIN');
...
-- Não está rodando atualmente, mas deve ser executado na criação da base
alter table lancamento add constraint FK69A57A28C1FC80 foreign key (idarquivo) references arquivo (id); 
...
update usuario set email = (select concat(login,'@hslife.com.br')); 
-- where tipoUsuario = 'ROLE_USER'; // trecho removido. Tarefa #976
...
-- executar caso ocorra erro na hora de excluir a coluna `idUsuario`
alter table itemdespensa change `idUsuario` `idUsuario` bigint null;  
...
-- Setando a senha do usuário 'admin' para A4m1n@01$
update usuario set senha = '982a3d231f726406cd28f29e1e0809e0ffadbb68abfde30031cd4ff7b23076f' where login = 'admin'; 
